  ...
  return (
    <div className="space-y-6">
      ...
      <form action={saveHero} className="bg-white rounded-lg border p-6 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            name="heading"
            rows={2}
            defaultValue={hero.heading ?? "Professional Cleaning Services for Contractors & Commercial Clients"}
          />
        </div>
        <div className="space-y-1.5">
          ...
          <Textarea
            name="subheading"
            rows={3}
            defaultValue={hero.subheading ?? "We handle everything from rough-clean on active construction sites to ongoing janitorial services for commercial spaces. Here's what we do:"}
          />
        </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
        <p className="font-medium mb-1">Individual service details</p>
        <p className="text-blue-700 mb-3">Each service (description, phases, included items) is managed in the Services editor.</p>
        <Button variant="outline" size="sm" asChild>
          <Link href="/admin/services" className="inline-flex items-center gap-1">
            Go to Services <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        </Button>
      </div>
    </div>

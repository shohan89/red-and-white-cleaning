  ...
  try {
    const lead = await prisma.lead.create({
      data: {
        ...
        message: `Deep Cleaning landing page request — service requested: ${service}`,
        howDidYouHear: 'Google Ads – Deep Cleaning Landing Page',
        status: 'NEW',
      },
    });
    await prisma.leadEvent.create({
      data: { leadId: lead.id, type: 'created', note: 'Lead created via /deep-cleaning landing page' },
    });
  } catch (dbError) {
    console.error('Failed to save deep-cleaning lead to DB:', dbError);
  }

  const { error } = await resend.emails.send({
    from: 'Red & White Cleaning <onboarding@resend.dev>',
    to: ['redandwhiteclean@gmail.com'],
    subject: `New Deep Cleaning Lead – ${service} (${name})`,
    html: `
      <h2 style="color:#c0392b;">New Deep Cleaning Landing Page Lead</h2>
      <table style="border-collapse:collapse;width:100%;font-family:sans-serif;font-size:15px;">
        ...
        <tr><td style="padding:8px 12px;font-weight:bold;">Source</td><td style="padding:8px 12px;">/deep-cleaning (Google Ads landing page)</td></tr>
      </table>
    `,
  });

  ...
  try {
    const lead = await prisma.lead.create({
      data: {
        ...
        message: `Residential Cleaning landing page request — service requested: ${service}`,
        howDidYouHear: 'Google Ads – Residential Cleaning Landing Page',
        status: 'NEW',
      },
    });
    await prisma.leadEvent.create({
      data: { leadId: lead.id, type: 'created', note: 'Lead created via /residential-cleaning landing page' },
    });
  } catch (dbError) {
    console.error('Failed to save residential-cleaning lead to DB:', dbError);
  }

  const { error } = await resend.emails.send({
    from: 'Red & White Cleaning <onboarding@resend.dev>',
    to: ['redandwhiteclean@gmail.com'],
    subject: `New Residential Cleaning Lead – ${service} (${name})`,
    html: `
      <h2 style="color:#c0392b;">New Residential Cleaning Landing Page Lead</h2>
      <table style="border-collapse:collapse;width:100%;font-family:sans-serif;font-size:15px;">
        ...
        <tr style="background:#f9f9f9;"><td style="padding:8px 12px;font-weight:bold;">Source</td><td style="padding:8px 12px;">/residential-cleaning (Google Ads landing page)</td></tr>
      </table>
    `,
  });
